...
export class VConsoleLogModel extends VConsoleModel {
  ...
  public getLogQuene(pluginId: string) {
  ...
  public addLog(
    ...
    const data = getLogDatasWithFormatting(item.origData || []);
    // prepare data
    const log: IVConsoleLog = {
      ...
      date: Date.now(),
      data,
      // exportData: safeStringify(data).length > 2000 ? [] : data,
      repeated: 0,
